...
public interface OntologiesApi extends TwinsApiClient.Api {
  ...
  @Headers({"Accept: */*",})
  List<OntologyRetrieve> ontologiesRetrieve(@Param("names") String names, @Param("loaded") Boolean loaded,
      @Param("includes") String includes);

  /**
   * Gets a list of ontologies
   *
   * Note, this is equivalent to the other <code>ontologiesRetrieve</code> method, but with the query
   * parameters collected into a single Map parameter. This is convenient for services with optional
   * query parameters, especially when used with the {@link OntologiesRetrieveQueryParams} class that
  ...
  List<OntologyRetrieve> ontologiesRetrieve(@QueryMap(encoded = true) Map<String, Object> queryParams);
  ...
  @Headers({"Accept: */*",})
  OntologyRetrieve ontologiesRetrieveById(@Param("id") Integer id, @Param("includes") String includes);
   ...
   * {@link OntologiesRetrieveByIdQueryParams} class that allows for building up this map in a fluent
   * style.
   *
   * @param id Ontology id (required)
   * @param queryParams Map of query parameters as name-value pairs
