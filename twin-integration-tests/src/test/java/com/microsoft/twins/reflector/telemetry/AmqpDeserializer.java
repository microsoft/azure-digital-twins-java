...
/***
 * This class implements a deserializer for bytes encoded with the AMQP type system as described by
 * the AMQP 1.0 standard. For more details of the type system, see
 * http://docs.oasis-open.org/amqp/core/v1.0/os/amqp-core-types-v1.0-os.html
 *
 * Because it is intended for use in applications using the Kafka client, it is based on the
 * Deserializer<> interface defined by the Kafka client. However, it is not really intended to
 * handle message bodies. Instead, it is intended to provide a convenient way for the application to
 * deserialize message headers in messages sent by AMQP clients.
 *
 * The AMQP type system is extensive and complex. Most AMQP clients only implement and support a
 * subset of it. The types that this deserializer supports are based on which types are emitted by
 * Microsoft's .NET and Java Event Hubs clients. Specifically, the types listed below (with the AMQP
 * type code) are supported:
 *
 * (0x00) described types -- only "com.microsoft:uri" is supported (0x40) null (0x73) UTF32
 * character (0x83) AMQP timestamp (64-bit milliseconds since the UNIX epoch) (0x98) UUID (0x41,
 * 0x42) boolean true and false (0x72, 0x82) 32- and 64-bit floating point numbers (0xA0, 0xB0) AMQP
 * binary (byte sequence) (0xA1, 0xB1) UTF-8 string (0xA3, 0xB3) AMQP symbol (USASCII string) (0x51,
 * 0x61, 0x71, 0x54, 0x81, 0x55) signed integers of 8, 16, 32, and 64 bits (0x50, 0x60, 0x70, 0x52,
 * 0x43, 0x80, 0x53, 0x44) unsigned integers of 8, 16, 32, and 64 bits
 *
 * Other types (various sizes of fixed-point decimal numbers, and lists, maps, and arrays) are not
 * supported.
 */
public class AmqpDeserializer implements Deserializer<Object> {
  ...
  /***
   * An AMQP described type consists of a descriptor (an AMQP symbol) and a value. This class packs
   * the two together for convenience when returning.
   */
   ...
   * @throws IllegalArgumentException if the bytes do not represent an AMQP described type or the
   *         described type is malformed or not recognized
   */
  ...
  /***
   * Deserialize any of the AMQP unsigned integer types. Because Java does not support unsigned,
   * return the value as a long, which can represent the unsigned range of any of the smaller types.
   * There is nothing larger than a 64-bit integer, so unsigned long is a matter of interpretation.
   * 
   * @param data AMQP-encoded bytes
  ...
  public Long deserializeUnsignedInteger(final byte[] data) throws IllegalArgumentException {
    final Result r = _deserializeUnsignedInteger(data);
  ...
  public Long deserializeSignedInteger(final byte[] data) throws IllegalArgumentException {
    final Result r = _deserializeSignedInteger(data);
  ...
  private int build2(final byte[] data, final int offset) {
    ...
    int dummy = (data[offset] & 0x000000FF);
    dummy <<= 8;
    dummy |= (data[offset + 1] & 0x000000FF);
    return dummy;
  }
  ...
  /***
   * Deserialize any of the AMQP floating point types. Return the value as a double, which can
  ...
  public Double deserializeFloatingPoint(final byte[] data) throws IllegalArgumentException {
    final Result r = _deserializeFloatingPoint(data);
    ...
    switch (data[0]) {
      ...
      default:
        throw new IllegalArgumentException(
            String.format("Bytes are not an AMQP floating point number, code %02X", data[0]));
    }
  ...
  /***
   * Deserializes a single UTF32 character. Java does not support UTF32 directly but has library
   * support to convert it to (potentially multi-byte sequences of) supported character sets.
   * 
   * @param data AMQP-encoded bytes
  ...
  private Result deserializeDouble(final byte[] data) throws IllegalArgumentException {
    ...
    final long longBits = build8(data, 1);
  ...
  private Result _deserializeUUID(final byte[] data) throws IllegalArgumentException {
    ...
    if ((data[0] != (byte) 0x98) || (data.length < 17)) {
      // Not a UUID
      throw new IllegalArgumentException(
          String.format("Bytes are not an AMQP UUID or data buffer too short, code %02X", data[0]));
    }
   ...
   * @throws UnsupportedOperationException if the bytes represent a known but unsupported AMQP type,
   *         or an unknown type
   */
  @Override
    ...
    switch (data[0]) {
      ...
      default:
        throw new UnsupportedOperationException(
            String.format("Deserialization not implemented for AMQP type %02X", data[0]));
    }
